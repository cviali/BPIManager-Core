...
class Captured extends React.Component<P, S> {
  ...
  shareWithNativeAPI = (targetImgUri: HTMLCanvasElement | null) => {
    ...
    targetImgUri.toBlob(async (blob) => {
      if (!blob)
        return alert("An error occured while converting image into blob!");
      ...
      navigator
        .share({
          text:
            `本日のスコア更新数：${updates.length || 0}件\n` +
            `総合BPI:${totalBPI}(前日比:${showBpiDist(
              totalBPI,
              lastDay
            )},前週比:${showBpiDist(
              totalBPI,
  ...
  getUpdatesToday = () =>
    new statMain(12).load().then((s) => s.updatedAtToday());
  getLastDay = () =>
    this.stat.eachDaySum(4, dayjs().subtract(1, "day").format());
  getLastWeek = () =>
    this.stat.eachDaySum(4, dayjs().subtract(1, "week").format());
  getRankPer = (rank: number) =>
    Math.round((rank / this.bpi.getTotalKaidens()) * 1000000) / 10000;
  ...
  upload = async () => {
    ...
    const profileURL = this.state.userName
      ? config.baseUrl + "/r/u/" + this.state.userName
      : "";
    ...
    if (
      ...
      !body.extended_entities.media[0] ||
    ...
    window.open(
      `https://twitter.com/intent/tweet?text=${
        ...
        `総合BPI:${totalBPI}(前日比:${showBpiDist(
          ...
          totalBPI,
          lastWeek
        )})%0a推定順位:${rank}位,皆伝上位${rankPer}％%0a`
      }${encodeURIComponent(profileURL)}%0a${encodeURIComponent(
        url
